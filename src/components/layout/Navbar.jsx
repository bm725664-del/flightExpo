...
export default function Navbar() {
  ...
  return (
    <>
      {/* Navbar */}
      <nav
        className={`fixed top-0 w-full z-50   transition-all duration-300 ${containerClasses}`}
      >
        <div className="flex justify-between items-center h-20 px-margin-desktop max-w-container-max mx-auto">
            ...
            {navLinks.map((link) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `transition-all pb-1 ${
                    isActive
                      ? "text-white bg-primary rounded-full px-4 py-1 shadow-[0_12px_40px_rgba(0,74,198,0.22)] ring-1 ring-primary/30"
                      : linkInactive
                  }`
                }
              >
          ...
          <div className="hidden md:flex items-center gap-stack-md">
            <div className="flex items-center gap-stack-sm mr-4">
              <span
                className={`material-symbols-outlined ${iconTextColor} cursor-pointer hover:scale-105 transition-transform`}
              >
                notifications
              </span>
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `px-4 py-3 rounded-full font-label-md transition-all ${
                      isActive
